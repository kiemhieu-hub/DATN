...
/**
 * Get appointment by ID for client.
 */
  ...
  async (appointmentId: string) => {
    ...
    const appointment =
      await Appointment.findById(appointmentId)
        ...
        .populate(
          "services.service",
          "name image group isActive staffType durationMinutes price"
        )
        .populate(
          "staffAssignments.barber",
          "fullName email phone role status"
...
export const cancelMyAppointment =
  async ({
    ...
    // Hoàn tiền cọc nếu hủy trước đủ thời gian quy định (trước 24 giờ).
    // Hủy trong ngày: vẫn được hoàn cọc theo yêu cầu.
...
export const rescheduleAppointment = async (
  ...
  assertObjectId(appointmentId, "Mã lịch hẹn không hợp lệ");
  assertObjectId(clientId, "Tài khoản không hợp lệ");

  if (!isDateFormatValid(input.appointmentDate)) {
    throw new AppError("Ngày đặt lịch không hợp lệ", 400);
  }

  if (!TIME_PATTERN.test(input.startTime)) {
    throw new AppError("Giờ bắt đầu không hợp lệ", 400);
  }
  ...
  if (!["PENDING", "CONFIRMED"].includes(appointment.status)) {
    throw new AppError("Chỉ có thể đổi lịch ở trạng thái chờ xác nhận hoặc đã xác nhận", 400);
  }

  if (
    isPastAppointment(input.appointmentDate, input.startTime)
  ) {
    throw new AppError("Không thể đổi lịch sang quá khứ", 400);
  }

  // Validate new time slot is available for all assigned barbers
  const relevantBarberIds = [
    String(appointment.barber),
    ...(appointment.staffAssignments || []).map((a) => String(a.barber)),
  ];

  for (const barberId of [...new Set(relevantBarberIds)]) {
    await validateBarberSchedule(barberId, input.appointmentDate, input.startTime, appointment.endTime);
    await validateAppointmentConflict(
      barberId,
  ...
  await recordAppointmentActivity({
    appointmentId: appointment._id,
    action: "RESCHEDULED",
    description: "Khách hàng đã đổi lịch hẹn",
    actorId: clientId,
    actorRole: "CLIENT",
    metadata: {
      previousDate,
  ...
  queueAppointmentLifecycleEmail(
    appointment,
    "RESCHEDULED",
    `Bạn đã đổi lịch hẹn từ ${formatDate(previousDate)} lúc ${previousTime} sang ${formatDate(input.appointmentDate)} lúc ${input.startTime}.`
  );

  void createStaffNotification({
    title: "Khách hàng đổi lịch",
    message: `${appointment.customer.fullName} đã đổi lịch ${appointment.appointmentCode} sang ${formatDate(input.appointmentDate)} lúc ${input.startTime}.`,
    kind: "APPOINTMENT_CHANGED",
    appointmentId: appointment._id,
  }).catch((error: unknown) => {
    console.error("Không thể tạo thông báo đổi lịch:", error);
  });
