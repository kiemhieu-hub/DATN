...
function Booking() {
  ...
  // Load available slots when selections change
  useEffect(() => {
    setStartTime("");
    if (
  ...
  // Calculate total with voucher (temporary display)
  const discountAmount = voucherCalculation?.discountAmount ?? 0;
  const totalAfterVoucher = subtotal - discountAmount;
  const depositRequired = totalAfterVoucher > 200000;
  const depositAmount = depositRequired ? Math.round(totalAfterVoucher * 0.3) : 0;
  ...
  const toggleHairService = (service: CatalogService) => {
    setError("");
    setHairBarberId("");
    setStartTime("");
    setVoucherCode("");
  ...
  const toggleCareService = (service: CatalogService) => {
    setError("");
    setStartTime("");
    setVoucherCode("");
    setVoucherCalculation(null);
    setVoucherMessage("");
    setCareServiceIds((current) => {
      if (current.includes(service.id)) return current.filter((id) => id !== service.id);
      if (!service.isExclusiveInGroup) return [...current, service.id];
      return [
        ...current.filter((id) => {
  ...
  // Load vouchers when services change
  useEffect(() => {
  ...
  const chooseVoucher = (voucher: AvailableVoucher): void => {
    ...
    setVoucherMessage(`Đã chọn ${voucher.code}. Giảm ${money(voucher.discountAmount)}đ trên hóa đơn tạm tính.`);
  ...
  // Separate services by staff type
      ...
      <form className="booking-layout" onSubmit={submit}>
        <main className="booking-main">
          ...
          <section className="booking-panel">
            <div className="booking-panel-title"><b>01</b><div><h2>Dịch vụ cắt tóc</h2><p>Chọn các dịch vụ cắt tóc, nhuộm, tạo kiểu.</p></div></div>
            {hairServiceGroups.map((group) => {
              ...
              return <div className="booking-service-group" key={group}><h3>{groupNames[group]}</h3><div className="booking-service-grid">
                {groupServices.map((service) => {
                  const selected = hairServiceIds.includes(service.id);
                  return <button type="button" key={service.id} className={`booking-service-card ${selected ? "selected" : ""}`} onClick={() => toggleHairService(service)}>
          ...
          <section className="booking-panel">
            <div className="booking-panel-title"><b>02</b><div><h2>Dịch vụ chăm sóc thư giãn</h2><p>Chọn các dịch vụ chăm sóc, massage, spa tóc.</p></div></div>
            {careServiceGroups.map((group) => {
              const groupServices = services.filter((service) => service.group === group);
              if (!groupServices.length) return null;
              return <div className="booking-service-group" key={group}><h3>{groupNames[group]}</h3><div className="booking-service-grid">
                {groupServices.map((service) => {
                  const selected = careServiceIds.includes(service.id);
                  return <button type="button" key={service.id} className={`booking-service-card ${selected ? "selected" : ""}`} onClick={() => toggleCareService(service)}>
                    <div className="booking-service-image">{service.image ? <img src={service.image} alt={service.name} /> : <span>THADS</span>}</div>
                    <div><strong>{service.name}</strong><p>{service.description}</p><small>{service.durationMinutes} phút · {money(service.price)}đ</small></div><i>{selected ? "✓" : "+"}</i>
                  </button>;
                })}
          ...
          {/* SECTION: Date & Barber Selection */}
          {serviceIds.length > 0 && <section className="booking-panel">
            <div className="booking-panel-title"><b>03</b><div><h2>Chọn ngày và Barber</h2><p>Chọn ngày trước, sau đó chọn Barber còn lịch phù hợp.</p></div></div>
            ...
            {needsHair && <BarberPicker title="Nhân viên làm tóc" items={hairBarbers} value={hairBarberId} onChange={(id) => changeBarber(id)} />}
          ...
          <section className="booking-panel">
            <div className="booking-panel-title"><b>04</b><div><h2>Chọn thời gian</h2><p>Khung giờ mờ đã có lịch; khung sáng còn có thể đặt.</p></div></div>
          ...
          <section className="booking-panel">
            <div className="booking-panel-title"><b>05</b><div><h2>Thông tin khách sử dụng</h2><p>Có thể sửa để đặt lịch hộ người khác.</p></div></div>
        ...
        <aside className="booking-summary">
          ...
          <div className="booking-time-summary">
            {needsHair && <div className="booking-time-item"><span>Cắt tóc</span><strong>{formatDuration(hairDuration)}</strong></div>}
            {needsCare && <div className="booking-time-item"><span>Chăm sóc</span><strong>{formatDuration(careDuration)}</strong></div>}
              ...
              <ul>
                {selectedServices.map((service) => (
                  <li key={service.id}>
                    ...
                    <button type="button" title={`Bỏ ${service.name}`} onClick={() => service.staffType === "HAIR" ? toggleHairService(service) : toggleCareService(service)}>✓</button>
                  </li>
                ))}
              </ul>
            )}
          ...
          {/* Voucher Details */}
          {voucherCalculation && (
            <div className="booking-voucher-details">
              <div className="booking-voucher-detail-row">
                <span>Tạm tính</span>
                <span>{money(subtotal)}đ</span>
              </div>
              <div className="booking-voucher-detail-row discount">
                <span>Giảm ({voucherCode})</span>
                <span>-{money(discountAmount)}đ</span>
              </div>
              <div className="booking-voucher-detail-row total">
                <span>Còn lại</span>
                <span>{money(totalAfterVoucher)}đ</span>
              </div>
            </div>
          )}

          <div className="booking-totals">
            <p className="grand-total"><span>Tổng tiền tạm tính</span><b>{money(voucherCalculation ? totalAfterVoucher : subtotal)}đ</b></p>
            {voucherCode && <small>Voucher <b>{voucherCode}</b> đã áp dụng giảm {money(discountAmount)}đ</small>}
            {depositRequired && <small>Đặt cọc 30%: <b>{money(depositAmount)}đ</b></small>}
          </div>
      ...
      {voucherOpen && (
        <div className="booking-voucher-modal" onMouseDown={() => setVoucherOpen(false)}>
          <section onMouseDown={(event) => event.stopPropagation()}>
            ...
            <h2>Chọn voucher</h2>
            <p>Voucher giảm trực tiếp trên hóa đơn tạm tính.</p>
