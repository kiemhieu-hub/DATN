...
function Payments() {
  ...
  useEffect(() => {
    ...
    void loadPayments();
  }, [authLoading, isAuthenticated, user, navigate, loadPayments]);
  ...
  return (
    <div className="admin-payments-page">
      <main className="admin-payments-container">
        ...
        <form className="payments-filters" onSubmit={handleSearch}>
          <input
            value={keyword}
          ...
          <select
            value={status}
            onChange={(event) => {
              setStatus(event.target.value as PaymentTransactionStatus | "ALL");
              setPage(1);
            }}
          >
            ...
            <tbody>
              {payments.length === 0 ? (
                ...
                payments.map((payment) => (
                  <tr key={payment._id}>
                    <td style={{ fontFamily: "monospace", fontWeight: 600 }}>{payment.transactionCode}</td>
                    <td>
                      <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
                        ...
                        {typeof payment.client !== "string" && payment.client.phone && (
                          <small style={{ color: "#888", fontSize: "12px" }}>{payment.client.phone}</small>
                        )}
                      </div>
                    </td>
                    <td>
                    ...
                    <td className="payments-amount">{formatMoney(payment.amount)}đ</td>
                    <td style={{ fontSize: "13px", color: "#999" }}>{formatDateTime(payment.paidAt)}</td>
                    <td>
      ...
      {selectedPayment && (
        <div className="payment-invoice-backdrop" onMouseDown={() => setSelectedPayment(null)}>
          <section className="payment-invoice" onMouseDown={(event) => event.stopPropagation()}>
            <button className="payment-invoice-close" onClick={() => setSelectedPayment(null)}>×</button>
            ...
            <div className="invoice-info">
              ...
              {typeof selectedPayment.client !== "string" && selectedPayment.client.phone && (
                <p><span>Điện thoại</span><strong>{selectedPayment.client.phone}</strong></p>
              )}
              {typeof selectedPayment.client !== "string" && selectedPayment.client.email && (
                <p><span>Email</span><strong>{selectedPayment.client.email}</strong></p>
              )}
              ...
              <p><span>Trạng thái</span><strong style={{ color: "#4caf50" }}>{statusLabels[selectedPayment.status]}</strong></p>
            </div>

            {typeof selectedPayment.appointment !== "string" && selectedPayment.appointment && (
            ...
            <button className="invoice-print-button" onClick={handlePrint}>
              🖨️ In hóa đơn
            </button>
          </section>
        </div>
