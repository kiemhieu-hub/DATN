...
const emptyForm: AdminServicePayload = {
  ...
  durationMinutes: 30, group: "HAIRCUT", image: "", isActive: true,
};
...
function Services() {
  ...
  const openEdit = (item: AdminService) => {
    ...
    setForm({ name: item.name, description: item.description, price: item.price, priceFrom: item.priceFrom,
      durationMinutes: item.durationMinutes, group: item.group, image: item.image, isActive: item.isActive });
  ...
  return <div className="admin-services-page"><main className="admin-services-container">
    ...
    <div className="service-table-wrap"><table><thead><tr><th>Dịch vụ</th><th>Nhóm</th><th>Giá</th><th>Thời lượng</th><th>Trạng thái</th><th>Thao tác</th></tr></thead>
      <tbody>{items.length === 0 ? <tr><td colSpan={6} className="empty">Không có dịch vụ phù hợp.</td></tr> : items.map((item) => <tr key={item.id}>
        <td><strong>{item.name}</strong><small>{item.description || "Chưa có mô tả"}</small></td>
        <td>{groups.find((g) => g.value === item.group)?.label}</td><td>{item.priceFrom && "Từ "}{money(item.price)}đ</td><td>{item.durationMinutes} phút</td>
